...
import {
  BuyerRequest,
  BuyerRequestStatus,
} from "../entities/buyer-request.entity"
...
export class BuyerRequestsService {
  ...
  async create(
    createDto: CreateBuyerRequestDto,
    userId: number,
  ...
  async findAll(
    query: GetBuyerRequestsQueryDto,
  ): Promise<PaginatedBuyerRequestsResponseDto> {
    const {
      page = 1,
      limit = 10,
      search,
    ...
    if (expiringSoon) {
      ...
      queryBuilder.andWhere("request.expiresAt <= :threeDaysFromNow", {
        threeDaysFromNow,
      })
    }
  ...
  async update(
    id: number,
    updateDto: UpdateBuyerRequestDto,
    userId: number,
  ): Promise<BuyerRequestResponseDto> {
    const request = await this.buyerRequestRepository.findOne({
      where: { id },
  ...
  async getSearchSuggestions(
    ...
    const suggestions = await this.buyerRequestRepository
      ...
      .andWhere("request.title ILIKE :query", {
        query: `%${query.trim()}%`,
      })
  ...
  async getPopularCategories(): Promise<
    Array<{ categoryId: number; count: number }>
  > {
    const categories = await this.buyerRequestRepository
      ...
      .andWhere(
        "(request.expiresAt IS NULL OR request.expiresAt > :now)",
        { now: new Date() },
      )
