...
export class StoreService {
  ...
  /**
   * Convert UUID string to a consistent numeric ID for legacy compatibility
   * This is a temporary solution until the Store entity is updated to use UUID
   */
  private convertUuidToNumericId(uuid: string): number {
    // Simple hash function to convert UUID to consistent numeric ID
    let hash = 0;
   ...
   */
  async createDefaultStore(sellerIdStr: string, sellerData: any): Promise<Store> {
    ...
    // Convert UUID to numeric seller ID for legacy compatibility
    const sellerId = this.convertUuidToNumericId(sellerIdStr);
   ...
   */
  async createStore(sellerIdStr: string, createStoreDto: CreateStoreDto): Promise<Store> {
    const seller = await this.userRepository.findOne({
      where: { id: sellerIdStr },
      relations: ['userRoles'],
    });
   ...
   */
  async getSellerStores(sellerIdStr: string): Promise<Store[]> {
    const sellerId = this.convertUuidToNumericId(sellerIdStr);
   ...
   * Update a store
   */
  async updateStore(storeId: number, sellerIdStr: string, updateStoreDto: UpdateStoreDto): Promise<Store> {
    const sellerId = this.convertUuidToNumericId(sellerIdStr);
    const store = await this.storeRepository.findOne({
   ...
   * Delete a store
   */
  async deleteStore(storeId: number, sellerIdStr: string): Promise<void> {
    const sellerId = this.convertUuidToNumericId(sellerIdStr);
    const store = await this.storeRepository.findOne({
      where: { id: storeId, sellerId },
    });
   ...
   */
  async getStoreStats(storeId: number, sellerIdStr: string): Promise<any> {
    const sellerId = this.convertUuidToNumericId(sellerIdStr);
    const store = await this.storeRepository.findOne({
      where: { id: storeId, sellerId },
    });
