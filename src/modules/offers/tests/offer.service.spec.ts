...
describe("OffersService", () => {
  ...
  const mockBuyerId = 1;
  const mockBuyerRequestId = "br-uuid-1";
  ...
  describe("create", () => {
    ...
    it("should create an offer successfully", async () => {
      ...
      expect(buyerRequestRepository.findOne).toHaveBeenCalledWith({ where: { id: mockBuyerRequestId } });
      expect(offerRepository.create).toHaveBeenCalledWith({ ...createOfferDto, sellerId: mockSellerId });
  ...
  describe("accept", () => {
    it("should accept an offer and reject others sequentially", async () => {
      ...
      expect(offerRepository.save).toHaveBeenCalledWith({ ...mockPendingOffer, status: OfferStatus.ACCEPTED });
    ...
    it("should throw BadRequestException if another offer is already accepted", async () => {
      offerRepository.findOne.mockResolvedValueOnce(mockPendingOffer);
      await expect(service.accept(mockOfferId, String(mockBuyerId))).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe("reject", () => {
    it("should reject a pending offer successfully", async () => {
      ...
      expect(offerRepository.save).toHaveBeenCalledWith({ ...mockPendingOffer, status: OfferStatus.REJECTED });
    });
    ...
    it("should throw BadRequestException if offer is not pending", async () => {
      ...
      await expect(service.reject(mockOfferId, String(mockBuyerId))).rejects.toThrow(BadRequestException);
    });
  });
});
