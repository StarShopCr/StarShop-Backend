...
describe('OffersService - RBAC + Wallet Ownership', () => {
  ...
  const mockSeller = {
    ...
    role: 'SELLER',              // If you use an enum: UserRole.SELLER
    isActive: true,
  ...
  const mockBuyer = {
    ...
    password: 'hashed-password',
    role: 'BUYER',               // UserRole.BUYER if enum
    isActive: true,
    isEmailVerified: true,
    offers: [],
    buyerRequests: [],
  ...
  describe('Seller wallet ownership validation', () => {
    ...
    describe('update offer', () => {
      it('should allow seller to update their own offer', async () => {
        ...
        userRepository.findOne.mockResolvedValue(mockSeller);
        offerRepository.save.mockResolvedValue({ ...mockOffer, amount: 200 } as Offer);
      ...
      it('should reject seller updating offer with different wallet', async () => {
        ...
        await expect(
          service.update(mockOffer.id, { amount: 200 }, +mockSeller.id)
      ...
      it('should reject non-seller (403) when trying to update offer', async () => {
        ...
        await expect(
          service.update(mockOffer.id, { amount: 200 }, +mockBuyer.id)
        ).rejects.toThrow(ForbiddenException);
      });
    });
  ...
  describe('Buyer wallet ownership validation', () => {
    describe('confirm purchase', () => {
      it('should allow buyer to confirm purchase with matching wallet', async () => {
        ...
        (dataSource.transaction as jest.Mock).mockImplementation(async (fn: (manager: any) => Promise<any>) => {
          const manager = {
  ...
  describe('Authorization edge cases', () => {
    it('should handle non-existent offers', async () => {
      ...
      const updateDto: UpdateOfferDto = { amount: 100 };

      await expect(service.update('non-existent', updateDto, +mockSeller.id)).rejects.toThrow(
        NotFoundException
      );
    });
